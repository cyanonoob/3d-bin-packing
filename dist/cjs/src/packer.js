...
class Packer {
    ...
    findFittedBin(i) {
        for (const bin of this._bins) {
            if (!bin.putItem(i, item_js_1.StartPosition)) {
                continue;
            }
    ...
    packToBin(b, items) {
        let b2 = undefined;
        const unpacked = [];
        const fit = b.putItem(items[0], item_js_1.StartPosition);
        if (!fit) {
